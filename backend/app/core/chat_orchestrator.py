...
class ChatOrchestrator:
    async def stream_chat(
        ...
        raw_answer = await llm_service.chat(system_prompt, user_message, max_tokens=4096)
        ...
        if not validation.approved:
            ...
            raw_answer = await llm_service.chat(system_prompt, repair_message, max_tokens=4096)
            answer_text, topology_data = self._parse_chat_response(raw_answer)
            validation = validate_chat_answer(answer_text, user_input=user_input, canvas_context=canvas_context)
...
输出格式：严格的 JSON，不要包含 markdown 代码块标记，不要添加任何前缀或后缀文字：
{"answer":"你的中文回复","topology":{"nodes":[{"id":"plc1","type":"plc","label":"S7-1200","x":250,"y":200,"status":"ok"}],"edges":[{"id":"e1","source":"plc1","target":"hmi1","protocol":"PROFINET"}]}}

ID 规则：使用简短 ID（如 plc1、cb2、rly3），不要用长描述性 ID。
label 精简到 15 字以内。
如果本轮没有涉及新元器件，topology 设为 null。
如果画布已有节点，新增 edges 应连接到已有节点 ID。
坐标按电气布局：电源上、PLC中、IO下、执行器右，间距 >= 80px 避免重叠。"""
    ...
    def _parse_chat_response(raw: str) -> tuple[str, dict[str, Any] | None]:
        ...
        # Try to salvage truncated JSON: find the last complete element and close
        if json_start >= 0:
            # Walk backwards from end to find the last valid closing point
            # Look for patterns like "}," or "]" that represent complete elements
            truncated = text[json_start:]
            # Find last occurrence of "}]" (end of a complete node/edge + start of closing)
            last_complete = truncated.rfind('"}')
            if last_complete > 0:
                salvage = truncated[:last_complete + 2] + ']}'
                try:
                    data = json.loads(salvage)
                    answer = str(data.get("answer", ""))
                    topology = data.get("topology")
                    if isinstance(topology, dict) and topology.get("nodes"):
                        return answer + "\n[注：回复被截断，已恢复部分拓扑数据]", topology
                except (json.JSONDecodeError, TypeError, AttributeError):
                    pass
            # Try without edges entirely — just take nodes and answer
            last_node_end = truncated.rfind('"}')
            last_array = truncated.rfind('],"edges"')
            if last_array > 0:
                salvage = truncated[:last_array] + ']}'
                try:
                    data = json.loads(salvage)
                    answer = str(data.get("answer", ""))
                    topology = data.get("topology")
                    if isinstance(topology, dict) and isinstance(topology.get("nodes"), list):
                        return answer + "\n[注：拓扑边被截断，已恢复节点数据]", {"nodes": topology["nodes"], "edges": []}
                except (json.JSONDecodeError, TypeError, AttributeError):
                    pass
