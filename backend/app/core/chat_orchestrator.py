...
class ChatOrchestrator:
    async def stream_chat(
        ...
        answer = await llm_service.chat(system_prompt, user_message, max_tokens=2048)
        validation = validate_chat_answer(answer, user_input=user_input, canvas_context=canvas_context)
        ...
        if not validation.approved:
            ...
            answer = await llm_service.chat(system_prompt, repair_message, max_tokens=2048)
            validation = validate_chat_answer(answer, user_input=user_input, canvas_context=canvas_context)

        if validation.approved:
            yield {"step": "核准校验通过，准备输出。", "node": "approval_gate"}
        else:
            answer = self._fallback_answer(user_input, canvas_context, validation.reason)
            validation = validate_chat_answer(answer, user_input=user_input, canvas_context=canvas_context)
...
输出要求：直接输出最终中文回复，不要输出 JSON，不要暴露内部步骤。"""
